...
description: >-
  ...
  适用于全新处理、继续同一次中断任务、从已封存判定重新生成工作簿，或把小额、大额、门店三类已封存任务续到
  紧接的下一会计日；不从旧台账反推聊天事实。
---
...
续接前一日工作目录、核对旧工作簿漂移或受保护替换累计工作簿前，完整读取
[references/roll-forward.md](references/roll-forward.md)。
...
- 大额群采用排除式选群：群名包含 `小额出` 或 `财务资料群` 时排除，其余群全部自动处理。名称含 `门店开票群` 的群
  同时按大额订单语义进入大额工作簿；另建 `store-ledger` 任务时，它又按内部 `+/-` 语义进入门店工作簿。两套判定互不复用。
  大额不是按金额判断，而是固定参与人员、换汇关系较稳定的群。
- 大额群采用与小额群相同的 `orders`、`open_orders`、资金明细和复杂关系格式；每个大额订单另填 `fund_type`。
  工作表明细使用小额群相同的 16 列，增加“订单状态”和“完成时间”；底部再按统计日期、资金类型、方向、精确汇率
  和当日状态统计。已确认与待确认订单都进入同一张资金汇总；当日尚未发生的一侧显示 `—`，金额无法确认显示
  “未确认”，真实净额为零才写数值 `0`。统计区使用深色分区标题、蓝底白字表头、交替浅色数据行、
...
- 普通资金图默认成功。只有图片或聊天明确显示失败、取消、拒绝、作废、无效或风控未完成时才记失败；明确取消的
  订单不能因出现图片而当作成交。
- 每个订单保存 `completed`、`pending_next_day` 或 `cancelled` 生命周期、状态历史、依据消息和原因；`completed`
  另存实际完成时间。聊天明确“明天继续”或截止时仍未换完、未回款时必须是 `pending_next_day`，即使已经出现部分付款
  和部分回款。它可以正常封存并进入工作簿；`open_orders` 只表示当前逐页阅读还没读完的对象。
...
前一日已经 `finish`、而下一日要继续同一批订单或票据时，不再使用 `start`，改用：

```powershell
python scripts/reconcile.py roll-forward <前一日work> <第二日聊天记录...> --previous-output <旧工作簿.xlsx> --work <全新work> --date YYYY-MM-DD
```

它只支持小额、大额和门店模式，要求日期紧接、前序原始快照与工作簿都在，并继承模式、时区、会计日边界、roster、
LINE 本机名和 OCR 设置。平台导出格式可以跨日变化；详细门禁、显式修订合同及复核范围见
[references/roll-forward.md](references/roll-forward.md)。
...
才发布。小额模式每个
实际群一张可见分表；大额首次任务只为当天存在订单的群建分表，跨日累计任务为累计期间内有订单的群建表，并在
相同明细格式下方追加按实际发生日拆分的统计；财务资料模式每个
...
- 跨日另阻止：前序任务未完成、日期重叠或断档、前序快照/旧工作簿丢失、群身份或消息正文/发送者/时间/引用冲突、
  媒体哈希冲突、旧表业务单元格漂移未被逐项显式修订完整解释、目标工作簿被占用或替换路径不是登记的前序工作簿。
