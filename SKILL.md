...
description: >-
  ...
  拆分、合并回款分摊与明确跨单补抵，并生成每群一张分表的 Excel 台账。适用于全新处理群聊导出、继续同一次核对任务或
...
- 脚本负责原始导出解析、群筛选、阅读进度、短标签映射、证据哈希、字段校验、金额运算、订单编号、Excel
  生成和回读核验。脚本只执行模型显式声明的同笔交易、拆分、合并回款分摊和补抵关系。
- 一张实际回款凭证明确同时结清多个订单时，模型使用群级 `settlement_allocations` 声明每单分配额；脚本
  校验原始回款金额守恒、客户一致、回款币种一致，并把同一物理凭证按分配额计入各订单，不能把整笔凭证
  强挂到其中一个订单后再用备注解释。
...
- 每条资金记录必须有收款方。模型打开原图，识别并原样记录实际收款方最具体的可见姓名、掩码账户或地址；
  同时填写 `payee_state=visible`。原图确实未显示时写 `payee=未显示/payee_state=not_shown`，看不清时写
  `payee=无法辨认/payee_state=unreadable`，现金写 `payee_state=cash` 并由脚本规范为 `现金`。脚本只校验并
  原样传递，不识别、不推断、不补值；收款方不参与订单状态判断。
- `截图所示人民币收款方`、`截图所示泰国收款账户`、`聊天指定USDT收款地址`、
  `固定金额二维码收款方`、`群内收款方`、`泰铢收款账户`、`USDT收款钱包` 等描述性占位内容非法，不能
  封存群判定。转账条目达到 10 条且超过一半为 `未显示/无法辨认` 时，必须重新打开这些原图复核，并把确认
  后的条目标识逐一写入 `unknown_payee_reviewed_entry_ids`，否则不能封存。
...
- 每条已录入资金记录必须由模型明确归入一个订单，或作为一笔合并回款由一个守恒的
  `settlement_allocations` 关系唯一占用；脚本不自动创建“未归单订单”。无法唯一判断时，由模型建立显式
  待确认订单并填写未知字段。
...
解析后的实际群名筛选。它创建固定聊天快照、平台前缀群标签和每群一份 v2.3 判定文件。禁止覆盖已有工作
...
4. 每批录入后运行 `review check`，让脚本规范字段、捕获新资金证据哈希，并检查返回的
   `missing_pricing_states`、`unknown_payee_review_required` 和 `unreviewed_unknown_payees`；封存前所有必需
   复核都必须完成。
...
使用 `apply_patch` 直接更新现有判定文件的语义字段。不要编写临时 Python 帮助函数批量生成收款方、
