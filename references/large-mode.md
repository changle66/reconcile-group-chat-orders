...
- `--mode large` 读取全部群，再排除群名中包含 `小额出` 或 `财务资料群` 的群；名称比较执行统一的 Unicode 和
  空白规范化，不使用白名单，也不逐群点选。
- `门店开票群` 也进入大额任务，按本文件的客户订单与资金流水语义独立判定；同一群另建 `store-ledger` 任务时按
  内部票据 `+/-` 判定。两份判定与工作簿互不复用，结果允许不同。
...
- 大额订单额外必填 `fund_type`，只能是 `wechat`、`alipay`、`bank_card`、`usdt` 或 `cash`。该字段只用于日终分类统计，
  不能代替资金流水、币种或收款方。
...
每个有订单的群生成一张分表。明细固定使用小额群相同的 16 列：记录类型、订单编号、客户昵称、换汇方向、
付款合计、汇率、收款方实际到账金额、流水币种、应回金额、内部实际回款合计、核对结果、备注、订单状态、完成时间、
收款方、聊天消息时间。
订单汇总行下面保留每条客户付款、内部回款、退款、追回及其他小额模式会显示的流水。

明细下方保留独立日终统计区，首列为“统计日期”，按 `实际发生日期 + 资金类型 + 换汇方向 + multiply/divide + 精确汇率 + 当日状态` 分组，记录笔数、
换出合计和换入合计。同一资金类型使用多个汇率或不同状态时必须拆行，不计算平均汇率，不把不同币种直接相加。
已确认和待确认订单或履约段都进入同一张资金汇总，并用“状态”列区分；待确认记录只汇总已有成功凭证支持的金额，
当日尚未发生的另一侧显示 `—`，金额已经发生但无法确认显示“未确认”，只有真实净额为零才写数值 `0`，不得从
另一侧倒推。一个跨日订单可在多天各有汇总行，资金按实际发生日归属，不因最终完成日搬移；失败或取消且没有成功
资金流水的事实只保留在明细和备注中。跨日流程见 [roll-forward.md](roll-forward.md)。
...
新任务不得创建 1.0 判定，也不得把 1.0 与 2.0 判定混在同一次任务中。已完成的 1.0 工作目录执行 `roll-forward`
时，只在每条旧 exchange 都有可验证的来源资金条目时转换为 2.0 完整订单，补建生命周期并列入定向复核；缺少来源
资金条目的旧记录不能安全迁移，必须先结合原聊天重新生成完整订单。
