...
- `--mode large` 读取全部群，再排除群名中包含 `小额出`、`财务资料群` 或 `门店开票群` 的群；名称比较执行统一的 Unicode 和
  空白规范化，不使用白名单，也不逐群点选。
...
明细下方保留独立日终统计区，按 `资金类型 + 换汇方向 + multiply/divide + 精确汇率 + 状态` 分组，记录笔数、
换出合计和换入合计。同一资金类型使用多个汇率或不同状态时必须拆行，不计算平均汇率，不把不同币种直接相加。
已确认和待确认订单或履约段都进入同一张资金汇总，并用“状态”列区分；待确认记录只汇总已有成功凭证支持的金额，
缺失的一侧明确显示“未确认”，不得写成 0 或从另一侧倒推。失败或取消且没有成功资金流水的事实只保留在明细和备注中。
