...
再次付款不自动代表新订单。前一订单尚未出现独立回款或明确结束，而客户表达的是再转、补转、多转、追加付款，
或将原应回金额调整为新的最终金额时，先把后续付款放回前一订单的完整上下文判断。只有聊天明确表示新单、另一单
或下一单，前一订单已经完成或取消，或者后续形成独立的报价—付款—回款链，才拆成新订单。金额能够凑合、时间
接近或使用同一付款地址都不能单独作为合并依据；证据不足时不得强行合并。
...
- 相邻订单边界；
- `order_continuity_review_candidates` 列出的订单对；该字段只表示需要重读所列消息区间，不表示脚本已经判定应合并；
- 失败、取消、未知字段或金额差异订单；
- `无法辨认` 的收款方；
- 拆分、合并、退款、追回、重复或跨单补抵；
- 疑似机械“一图一单”的告警订单。
